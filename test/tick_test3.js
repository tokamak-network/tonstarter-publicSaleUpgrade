...
describe("Tick test", () => {
    ...
    let swapWTONAmount = ethers.utils.parseUnits("100", 27);
    ...
    describe("#2. tick & timeWeightedAverageTick check", () => {
        it("#2-1. first tick & timeWeightedAverageTick check", async () => {
            ...
            let toszero = await tos.balanceOf(wtonhave.address)
            expect(toszero).to.be.equal(0);
        })
        ...
        it("#2-4.round3 : swap & check", async () => {
            const block = await ethers.provider.getBlock('latest')
            reversPath = encodePath(
                [config.addressinfo.tos,config.addressinfo.wton],
                [FeeAmount.MEDIUM]
            )
            let swapTOSAmount = await tos.balanceOf(wtonhave.address);
            console.log("swapTOSAmount : ",Number(swapTOSAmount));
            params = getExactInputParams(
                wtonhave.address,
                reversPath,
                swapTOSAmount,
            ...
            await tos.connect(wtonhave).approve(uniswapRouter.address,swapTOSAmount);
            let tx = await uniswapRouter.connect(wtonhave).exactInput(params);
            await tx.wait();
        ...
        it("#2-22.round21 : swap & check", async () => {
            const block = await ethers.provider.getBlock('latest')
            let swapTOSAmount = await tos.balanceOf(wtonhave.address);
            console.log("swapTOSAmount : ",Number(swapTOSAmount));
            params = getExactInputParams(
